...
private extension TagCellLayout {
	...
	var currentTagFrame: CGRect {
		guard let info = currentTagLayoutInfo?.layoutAttribute, let spacing = currentTagLayoutInfo?.itemSpacing  else { return .zero }
		var frame = info.frame
        frame.origin.x += info.bounds.width + spacing
		return frame
	}
